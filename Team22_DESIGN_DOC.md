...
## Section 7 — Task 6 Extensions (Bonus)

Our team implemented two major extensions to demonstrate both backend transaction safety and frontend/RAG integration.

### 7.1 Extension A: Promo Code Subsystem & Crowdedness (Owner: ds0w0)

#### Motivation
Adding a promotional code subsystem provides significant commercial value to TransitFlow. It enables the operator to run targeted marketing campaigns and seasonal discounts while safely enforcing strict redemption limits (maximum uses and expiry dates) under high-concurrency booking environments. Inspired by the success of the 'Taipei MRT GO' application, we also added a dynamic **Crowdedness Indicator** to rail schedules and a **Frequent Flyer Reward** calculation that tier-discounts users based on their monthly travel volume.
...
To validate a code during booking, we use a pessimistic lock (`FOR UPDATE`) to prevent race conditions:
...
**Expected Output:** `[{"code": "TRANSIT10", "discount_percent": 10.00, "max_uses": 100, "current_uses": 0, "expiry_date": "2027-12-31"}]`

#### Testing Evidence

* **Promo Code Validation**: Executing `query_validate_promo_code('TRANSIT10')` successfully returns a JSON record containing `discount_percent: 10.00`.
* **Concurrency Locking Test**: Multiple simulated booking requests targeting the same promotion bucket successfully locked the row for update, ensuring `current_uses` increments linearly without race condition overwrites.

---

### 7.2 Extension B: Trip History Panel & RAG Expansion (Owner: yikes0000)

#### Motivation

The original TransitFlow UI is entirely chat-based. A user asking "show my bookings" receives a one-time text reply that scrolls away. The **My Bookings** tab solves this by providing a dedicated, persistent, tabular view of the user's booking history queried directly from PostgreSQL. This adds a genuinely new interaction mode that bypasses the LLM entirely for a specific, well-defined query.
...
```sql
-- Fetch national rail bookings
SELECT booking_id, schedule_id, origin_station_id, destination_station_id, travel_date, amount_usd, status
FROM national_rail_bookings WHERE user_id = %s ORDER BY booked_at DESC;

-- Fetch metro trips
SELECT trip_id, schedule_id, origin_station_id, tap_in_at, fare_usd, status
FROM metro_travel_history WHERE user_id = %s ORDER BY tap_in_at DESC;

```

#### RAG Knowledge Base Extension

In addition to the UI panel, the policy knowledge base was expanded by extending `train-mock-data/travel_policies.json` with 4 new sections (`children_and_family`, `passenger_conduct`, `season_tickets_and_passes`, `special_services`). This increased the total embedded policy documents from **15 to 21**, allowing the RAG assistant to answer questions about children's fares and night services that were previously outside its knowledge base.

#### Testing Evidence

* **Unauthenticated state:** When a user clicks Refresh without logging in, the panel shows a warning message `⚠️ Please log in to view your booking history.`
* **Authenticated state:** After logging in and clicking Refresh, the panel dynamically renders `gr.Dataframe` tables displaying full historical logs fetched from the relational layer. (See `screenshots/task6_authenticated.png` in the repository).
